...
候选按时事、合肥本地、人物文化、轻松趣闻、社区、技术与学术等编辑栏目组织。
系统按发布时间、来源可信度和热度评估候选，并在栏目之间轮换；论文没有固定加分，
学术栏目也有独立上限。新闻检索词、地区、时效窗口和栏目配额都可在 `sources.yaml`
中配置，不需要修改代码。需要代理的设备可单独配置 Discovery 代理；凭据仍不进入仓库。
